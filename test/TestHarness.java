...
public abstract class TestHarness {
	...
	public static class IncorrectAnswer extends Exception {
		public IncorrectAnswer(String why) {
			super(why);
		}
	...
	public static class InvalidResults extends IncorrectAnswer {
		...
		public InvalidResults(Collection<Datum[]> missingResults,
				Collection<Datum[]> unexpectedResults, List<String> colNames) {
		...
		public String getDetails() {
			...
			if (unexpectedResults.size() > 0) {
				...
				err.append(TestHarness.stringOfOutput(unexpectedResults,
						colNames));
				err.append("\n");
			}
			return err.toString();
	...
	public static void validateResults(Collection<Datum[]> results,
			List<Datum[]> expectedResults, List<String> colNames)
			...
			for (i = 0; i < expectedResults.size(); i++) {
				...
				if (row.length != cmpRow.length) {
					throw new IncorrectAnswer(
							"Result has an incorrect number of fields");
				}
				int j;
				for (j = 0; j < cmpRow.length; j++) {
					if (!row[j].equals(cmpRow[j])) {
						break;
					}
				}
				if (j >= cmpRow.length) {
		...
		if ((expectedResults.size() > 0) || (unexpectedResults.size() > 0)) {
			throw new InvalidResults(expectedResults, unexpectedResults,
					colNames);
		}
	}

	public static void testQuery(Map<String, Schema.TableFromFile> tables,
			List<Datum[]> expectedResults, PlanNode query) {
		try {
			List<String> colNames = new ArrayList<String>();
			for (Schema.Var v : query.getSchemaVars()) {
				colNames.add(v.name);
			}
			validateResults(Sql.execQuery(tables, query), expectedResults,
					colNames);
